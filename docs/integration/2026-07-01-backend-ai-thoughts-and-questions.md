...
### 8. UserAccount 缺 realName/phone/email，是真实设计缺陷（P1，需产品决策）

**现象**：B3 任务书要求"更新姓名/手机/邮箱"，但 `user_account` 表只有 username/passwordHash/状态字段，没有任何联系方式或真实姓名字段。医生有 `Doctor` 表、患者有 `Patient` 表存这些，但**纯管理员账号无处存放**。

**影响**：第一阶段 B3 只交付"角色更新"，无法满足任务书完整要求。

**需要讨论**：
- 方案 A：扩展 `user_account` 表加 `real_name`/`phone`/`email` 列（需走数据库基线变更流程）。
- 方案 B：新建 `user_profile` 表（1:1），与 `user_account` 解耦。
- 方案 C：复用 `Doctor`/`Patient` 表，管理员账号也建一条 Doctor 记录（语义混乱，不推荐）。
- 建议方案 A 最直接，但需确认是否影响现有登录流程（登录只用 username/passwordHash，加字段不影响）。

**这是 B3 唯一的遗留功能缺口，建议优先推进。**
...
| # | 问题 | 需谁确认 | 优先级 | 状态 |
|---|---|---|---|---|
| Q1 | JWT 校验是否检查 tokenVersion？若否，B3 锁定/禁用/重置密码对已登录会话不立即生效 | 后端安全 | P1 | 待确认 |
| Q2 | UserAccount 是否扩展 realName/phone/email 字段？B3 完整交付依赖此决策 | 产品 + DBA | P1 | 待确认 |
| Q3 | OpenAPI 主契约同步流程：谁负责、何时同步？本次 12 个新接口未进契约 | 联调 AI / 契约组 | P1 | 待确认 |
| Q4 | 分页主参数名最终用 `pageSize` 还是 `size`？需收敛 | 契约组 | P2 | 待确认 |
| Q5 | B2 设备字段映射（category→type 等）是否接受？还是要扩展 device 表 | 联调 AI + DBA | P2 | 待确认 |
| Q6 | B3 创建 DOCTOR 时，前端表单字段集合需对齐（departmentId/doctorName/doctorTitle/specialty...） | 前端 AI | P2 | 待确认 |
| Q7 | 前端如何感知 B6 DashboardSummary 新字段？是否需要版本化接口 | 前端 AI | P2 | 待确认 |
| Q8 | B3 高敏感操作是否要补审计日志？审计范围和存储方式 | 后端 + 安全 | P2 | 待确认 |
| Q9 | 权限校验风格是否统一？何时收敛为 @PreAuthorize 为主 | 后端架构 | P3 | 待确认 |
| Q10 | DoctorService.createDoctor 在 B3 复用时是否有副作用（发通知、初始化数据）需注意 | 后端 | P3 | 待确认 |
...
**做得到位的**：
- 严格遵循任务书"不擅自改 DB 基线 / 不擅自扩 OpenAPI / 不擅自改前端"三条红线，所有字段差异都在文档标注并降级实现。
- 全量测试通过（381 个），无回归。
- B3 状态语义与现有 `AuthService.checkAccountStatus` 闭环验证（禁用/锁定登录会被拒），不是凭空实现。
- B8 用工具类兼容分页参数，没有破坏老接口。

**做得保守的**：
- B3 因 UserAccount 缺字段，第一阶段只交付角色更新，姓名/手机/邮箱缺口留待决策——这是遵循红线的正确取舍，但意味着 B3 未 100% 满足任务书。
- B6 用 record 加字段是前向不兼容，没有做版本化——因为任务书要求扩展返回，未要求版本化，权衡后选择最小改动。

**最需要立刻跟进的**：
1. **Q1（JWT tokenVersion 校验）**——这是安全语义是否成立的关键，不确认则 B3 重置密码/锁定的"立即生效"承诺存疑。
2. **Q2（UserAccount 字段扩展）**——B3 功能完整性的唯一阻塞项。
3. **Q3（OpenAPI 契约同步）**——12 个新接口未进契约，前端无法正式对接。

---

## 八、代码审查补充发现（2026-07-01 复查）

对 B1–B8 逐文件复查后，发现以下此前未记录的问题。所有问题均为非阻断，B1–B8 均符合任务书验收标准、381 个测试全过。
...
| B3 | 符合（姓名/手机/邮箱因表结构限制未交付，已文档说明） | 5 端点全管理员权限，createUser 区分 ADMIN/DOCTOR/PATIENT，changeStatus 三动作，resetPassword 设 hash+mustChange+tokenVersion++ | 14 个测试 | 通过 |
...
全量测试：`Tests run: 381, Failures: 0, Errors: 0, Skipped: 0 — BUILD SUCCESS`。
...
- **Q2 UserAccount 扩字段（realName/phone/email）**：涉及改数据库基线，任务书明文禁止擅自改，需联调 AI/DBA。B3 完整交付的唯一阻塞项。
- **Q3 OpenAPI 主契约同步**：任务书明确由联调 AI 负责，12 个新接口待入契约。
- **Q4 分页主参数名**：任务书明确由契约组决定，B8 仅做兼容。
- **第 16 条（角色变更 DOCTOR↔ADMIN 联动）**：涉及业务规则（软删 Doctor 档案？禁止互转？），任务书"不擅自扩展医疗业务规则"，需产品决策。
- **第 13 条（B3 审计日志）**：需扩展 AuditService 或审计表结构，任务书未要求，保留待定。
- **第 1/9 条（权限校验风格统一）**：架构级收敛，不在本次范围。
- **Q6/Q7（前端字段对齐）**：前端 AI 职责。

### 测试结果

本次自处理后全量测试：`Tests run: 387, Failures: 0, Errors: 0, Skipped: 0 — BUILD SUCCESS`（381 + 1 设备 departmentId 测试 + 5 AdminUserController 测试）。
