...
class AuditControllerTest {
    ...
    // ============== AI 调用列表测试（listInvocations）==============

    @Test
    @DisplayName("AI 调用列表 - 默认参数应返回分页结构")
    void listInvocations_shouldReturnPage() throws Exception {
        AIInvocation invocation = AIInvocation.builder()
                .id(1L)
                .capability("TRIAGE")
                .businessType("TRIAGE")
                ...
                .updatedAt(LocalDateTime.of(2026, 6, 25, 10, 0, 5))
                .build();
        Page<AIInvocation> page = new PageImpl<>(List.of(invocation), PageRequest.of(0, 20), 1);
        when(auditService.listInvocations(any(), any(), any(), any(), any(), any(Pageable.class)))
                .thenReturn(page);

        mockMvc.perform(get("/api/audit/ai/invocations"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"))
                .andExpect(jsonPath("$.data.items").isArray())
                .andExpect(jsonPath("$.data.items[0].id").value(1))
                .andExpect(jsonPath("$.data.items[0].capability").value("TRIAGE"))
    ...
    @Test
    @DisplayName("AI 调用列表 - 空结果应返回空分页")
    void listInvocations_emptyResult_shouldReturnEmptyPage() throws Exception {
        Page<AIInvocation> emptyPage = new PageImpl<>(
                List.of(), PageRequest.of(0, 20), 0);
        when(auditService.listInvocations(any(), any(), any(), any(), any(), any(Pageable.class)))
    ...
    @Test
    @DisplayName("AI 调用列表 - 携带筛选参数应正确传递给 Service")
    void listInvocations_withFilters_shouldPassParams() throws Exception {
        Page<AIInvocation> emptyPage = new PageImpl<>(
                List.of(), PageRequest.of(0, 20), 0);
        when(auditService.listInvocations(any(), any(), any(), any(), any(), any(Pageable.class)))
                .thenReturn(emptyPage);
        ...
        mockMvc.perform(get("/api/audit/ai/invocations")
                        .param("capability", "TRIAGE")
                        .param("success", "true")
                        .param("businessType", "TRIAGE"))
                .andExpect(status().isOk());

        org.mockito.ArgumentCaptor<String> capabilityCaptor =
                org.mockito.ArgumentCaptor.forClass(String.class);
        org.mockito.ArgumentCaptor<Boolean> successCaptor =
                org.mockito.ArgumentCaptor.forClass(Boolean.class);
        org.mockito.ArgumentCaptor<String> businessTypeCaptor =
                org.mockito.ArgumentCaptor.forClass(String.class);
        verify(auditService).listInvocations(
                capabilityCaptor.capture(), successCaptor.capture(),
                businessTypeCaptor.capture(),
                any(), any(), any(Pageable.class));

        org.assertj.core.api.Assertions.assertThat(capabilityCaptor.getValue())
                .as("capability 应原样传递给 Service")
                .isEqualTo("TRIAGE");
        org.assertj.core.api.Assertions.assertThat(successCaptor.getValue())
                .as("success 应原样传递给 Service")
                .isTrue();
        org.assertj.core.api.Assertions.assertThat(businessTypeCaptor.getValue())
                .as("businessType 应原样传递给 Service")
                .isEqualTo("TRIAGE");
    }

    // ============== 审计日志查询路由测试（queryLogs）==============

    @Test
    @DisplayName("审计日志查询 - 按目标查询应路由到 findByTarget")
    void queryLogs_byTarget_shouldReturnDto() throws Exception {
        AuditLog log = AuditLog.builder()
                .id(1L)
                .operatorId(100L)
                .operatorType("USER")
                ...
                .targetId(200L)
                .result("SUCCESS")
                .traceId("trace-target-001")
                .createdAt(LocalDateTime.of(2026, 6, 25, 10, 0, 0))
                .build();
        Page<AuditLog> page = new PageImpl<>(List.of(log), PageRequest.of(0, 20), 1);
        when(auditService.findByTarget(eq("USER_ACCOUNT"), eq(200L), any(Pageable.class)))
                .thenReturn(page);

        mockMvc.perform(get("/api/audit/logs")
                        .param("targetType", "USER_ACCOUNT")
                        .param("targetId", "200"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"))
                .andExpect(jsonPath("$.data.items[0].id").value(1))
                .andExpect(jsonPath("$.data.items[0].targetType").value("USER_ACCOUNT"))
                .andExpect(jsonPath("$.data.items[0].targetId").value(200))
                .andExpect(jsonPath("$.data.items[0].action").value("UPDATE"));

        verify(auditService).findByTarget(eq("USER_ACCOUNT"), eq(200L), any(Pageable.class));
    }

    @Test
    @DisplayName("审计日志查询 - 按动作与时间范围查询应路由到 findByActionAndTimeRange")
    void queryLogs_byActionAndTimeRange_shouldReturnDto() throws Exception {
        AuditLog log = AuditLog.builder()
                .id(2L)
                .operatorId(100L)
                .operatorType("USER")
                ...
                .createdAt(LocalDateTime.of(2026, 6, 26, 9, 0, 0))
                .build();
        Page<AuditLog> page = new PageImpl<>(List.of(log), PageRequest.of(0, 20), 1);
        when(auditService.findByActionAndTimeRange(
                eq("LOGIN"), any(), any(), any(Pageable.class))).thenReturn(page);

        mockMvc.perform(get("/api/audit/logs")
                        .param("action", "LOGIN")
                        .param("startDate", "2026-06-25")
                        .param("endDate", "2026-06-27"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"))
                .andExpect(jsonPath("$.data.items[0].id").value(2))
                .andExpect(jsonPath("$.data.items[0].action").value("LOGIN"))
                .andExpect(jsonPath("$.data.items[0].traceId").value("trace-action-001"));

        // 校验时间范围：start=startDate.atStartOfDay()，end=endDate.plusDays(1).atStartOfDay()
        ...
        org.mockito.ArgumentCaptor<java.time.LocalDateTime> endCaptor =
                org.mockito.ArgumentCaptor.forClass(java.time.LocalDateTime.class);
        verify(auditService).findByActionAndTimeRange(
                eq("LOGIN"), startCaptor.capture(), endCaptor.capture(), any(Pageable.class));

        org.assertj.core.api.Assertions.assertThat(startCaptor.getValue())
                .as("start 应为 startDate 的当天 00:00:00")
                .isEqualTo(LocalDateTime.of(2026, 6, 25, 0, 0, 0));
        org.assertj.core.api.Assertions.assertThat(endCaptor.getValue())
                .as("end 应为 endDate 次日的 00:00:00（开区间）")
                .isEqualTo(LocalDateTime.of(2026, 6, 28, 0, 0, 0));
    }
}
